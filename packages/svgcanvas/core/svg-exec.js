...
export const init = canvas => {
  ...
  console.log('🔍 Función exportAdvancedPDF existe:', typeof exportAdvancedPDF)
  svgCanvas.exportAdvancedPDF = exportAdvancedPDF // Generates an advanced PDF with layer support using pdf-lib
  console.log('✅ exportAdvancedPDF registrado, tipo:', typeof svgCanvas.exportAdvancedPDF)
...
const exportAdvancedPDF = async (
  ...
  console.log('🚀 exportAdvancedPDF INICIADA')
  console.log('Parámetros recibidos:', { windowName, outputType, options })

  try {
    // Temporalmente, usar la exportación PDF estándar como fallback
    console.log('📄 Usando exportación PDF estándar como fallback temporal')
    const result = await exportPDF(windowName, outputType)

    // Añadir flag para indicar que es una exportación avanzada temporal
    result.advanced = true
    result.options = options
    result.fallback = true

    console.log('✅ PDF exportado exitosamente (modo fallback)')
    return result
  } catch (error) {
    console.error('❌ Error en exportAdvancedPDF:', error)
    throw error
  }
}
